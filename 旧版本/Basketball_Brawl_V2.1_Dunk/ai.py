...
from constants import (
    ...
    AI_DUNK_APPROACH_DISTANCE, AI_DUNK_TAKEOFF_DISTANCE,
)
...
def update_ai(player, ball, opponent):
    ...
    if ball.holder is player and ball.state == "held":
        ...
        # 篮下优先冲筐：进入起跳距离后跳起，Player.try_dunk 会在空中完成扣篮。
        if 0 < distance_from_rim <= AI_DUNK_APPROACH_DISTANCE:
            direction = -1
            if distance_from_rim <= AI_DUNK_TAKEOFF_DISTANCE and player.on_ground:
                want_jump = True
            player.ai_shot_target = None
        elif player.ai_shot_target is None:
        ...
        if player.ai_shot_target is not None:
